        ...
        stage('Deploy') {
            steps {
                ...
                sh """
                    ...
                    docker stop ddd-api;
                    docker container run -d -t -e ENCRYPTOR_PASSWORD=${env.ENCRYPTOR_PASSWORD} --restart always --network=ddd-10-wable-server_default --rm --name ddd-api ${repository}:main;
                """
            }
        }
        stage('Notification') {
