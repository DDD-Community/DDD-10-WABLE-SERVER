...
public abstract class BaseTimeEntity {
    ...
    @Column(name = "create_date", updatable = false) // 해당 컬럼은 수정되지 않는다는 의미
    // update 시에 Null 되는 경우 방지
    private LocalDateTime createDate;

    @LastModifiedDate
    @Column(name = "modified_date", insertable=false) // 해당 컬럼은 삽입에서 제외된다는 의미(DB Insert에서 제외). 삽입 시점에서는 비워져있어야하므로 false
